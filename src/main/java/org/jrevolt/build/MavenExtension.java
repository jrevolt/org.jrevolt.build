...
public class MavenExtension extends AbstractMavenLifecycleParticipant {
	...
	public void afterSessionStart(MavenSession session) throws MavenExecutionException {
		...
		if (isRelease && tag == null) {
			throw new MavenExecutionException("RELEASE build requires tag but none has been detected nor provided.",
														 (Throwable) null);
		}
	...
	String detectReleaseTag(String commitId) {
		...
		while (it.hasNext()) {
			...
			if (line.startsWith(commitId)) {
				...
				if (pbuild.matcher(tag).matches()) {
					continue;
				}
				if (!prelease.matcher(tag).matches()) {
					continue;
				}
